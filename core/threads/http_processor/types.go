...
type Thread struct {
	...
	C13       chan common.ProcessorRequest
	C14       chan common.ProcessorResponse
...
func NewThread(logger *utils.Logger, channels ...any) (*Thread, error) {
	...
	thread.C13, ok = (channels[1]).(chan common.ProcessorRequest)
	if !ok {
		return nil, errors.New("expected type 'chan ProcessorRequest' in index 1")
	}

	thread.C14, ok = (channels[2]).(chan common.ProcessorResponse)
	if !ok {
