...
import (
	...
	"github.com/GabeCordo/etl/core/components/module"
...
func (provisionerThread *Thread) ProcessAddModule(request *threads.ProvisionerRequest) {
	...
	for _, clusterWrapper := range registeredClusters {
		...
		if helperImplementation, ok := (clusterImplementation).(cluster.Helper); ok {
			helper, _ := NewHelper(moduleWrapper.Identifier, clusterWrapper.Identifier,
				provisionerThread.C9, provisionerThread.C11)
