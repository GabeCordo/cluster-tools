...
import (
	...
	"errors"
	"fmt"
	"github.com/Sentmint/pops/internal/core/thread/cache"
	...
	http_client "github.com/Sentmint/pops/internal/core/thread/rest"
...
func (config *Config) ToYAML(path string) {
	...
	// if a pops-core already exists, delete it
...
func (config *Config) Store() bool {
	// verify that the pops-core file we initially loaded from has not been deleted
...
func YAMLToETLConfig(config *Config, path string) error {
	...
	if err != nil {
		// the file is not a JSON or is a malformed (fields missing) pops-core
		log.Println(err)
		return err
	}
