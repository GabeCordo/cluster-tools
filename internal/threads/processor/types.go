...
type Thread struct {
	...
	C5 <-chan common.ThreadRequest  // Processor rec req from the http_client thread
	C6 chan<- common.ThreadResponse // Processor sending rsp to the http_client thread

	C7 <-chan common.ThreadRequest  // Processor rec req from the http_processor thread
	C8 chan<- common.ThreadResponse // Processor sending rsp to the http_processor thread

	C11 chan<- common.ThreadRequest  // Processor sending req to the database thread
	C12 <-chan common.ThreadResponse // Processor rec rsp from the database thread

	C13 chan<- common.ThreadRequest  // Processor thread sending req to the supervisor thread
	C14 <-chan common.ThreadResponse // Processor thread rec rsp from the supervisor thread

	C18 <-chan common.ThreadRequest  // Processor rec req from the scheduler thread
	C19 chan<- common.ThreadResponse // Processor sending rsp to the scheduler thread
