...
import (
	...
	provisioner_component "github.com/GabeCordo/etl/framework/components/provisioner"
...
func NewCore(configPath string) (*Core, error) {
	...
	core.DatabaseThread, err = database.NewThread(databaseLogger, core.interrupt, core.C1, core.C2, core.C3, core.C4, core.C7, core.C8)
	if err != nil {
		return nil, err
	}
...
func (core *Core) Cluster(identifier string, mode cluster.EtlMode, implementation cluster.Cluster, config ...cluster.Config) {
	...
	defaultModule, _ := p.GetModule(provisioner_component.DefaultFrameworkModule) // the default framework module should always be found

	clusterWrapper, _ := defaultModule.AddCluster(identifier, mode, implementation)
	clusterWrapper.UnMount()

	if len(config) == 1 {
		d := database.GetDatabaseInstance()
		d.StoreClusterConfig(provisioner_component.DefaultFrameworkModule, config[0])
	}
}
